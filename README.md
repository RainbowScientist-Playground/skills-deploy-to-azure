...
jobs:
  build:
    ...
    if: contains(github.event.pull_request.labels.*.name, 'stage')A lot of things go into delivering "continuously". These things can range from culture and behavior to specific automation. In this exercise, we're going to focus on the deployment part of our automation.
...
### :keyboard: Activity 1: Configure `name: Stage the app
...
jobs:
  build:
    ...
    if: contains(github.event.pull_request.labels.*.name, 'stage')_TOKEN` permissions
